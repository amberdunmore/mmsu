...
typedef struct model_internal {
  ...
  double rD;
  double res_time;
  double resistance_asymptomatic_ratio;
...
SEXP model_create(SEXP user) {
  ...
  internal->resistance_asymptomatic_ratio = NA_REAL;
...
SEXP model_contents(SEXP internal_p) {
  ...
  SEXP contents = PROTECT(allocVector(VECSXP, 61));
  ...
  SET_VECTOR_ELT(contents, 43, ScalarReal(internal->resistance_asymptomatic_ratio));
  SET_VECTOR_ELT(contents, 44, ScalarReal(internal->resistance_baseline_ratio));
  SET_VECTOR_ELT(contents, 45, ScalarReal(internal->resistance_cleared_ratio));
  SET_VECTOR_ELT(contents, 46, ScalarReal(internal->resistance_dur_mult));
  SET_VECTOR_ELT(contents, 47, ScalarReal(internal->resistance_failed_ratio));
  SET_VECTOR_ELT(contents, 48, ScalarReal(internal->resistance_trans_mult));
  SET_VECTOR_ELT(contents, 49, ScalarReal(internal->rT_r_cleared));
  SET_VECTOR_ELT(contents, 50, ScalarReal(internal->rT_r_failed));
  SET_VECTOR_ELT(contents, 51, ScalarReal(internal->rT_s));
  SET_VECTOR_ELT(contents, 52, ScalarReal(internal->S0));
  SET_VECTOR_ELT(contents, 53, ScalarReal(internal->Sv0));
  SET_VECTOR_ELT(contents, 54, ScalarReal(internal->T_r_cleared0));
  SET_VECTOR_ELT(contents, 55, ScalarReal(internal->T_r_failed0));
  SET_VECTOR_ELT(contents, 56, ScalarReal(internal->T_s0));
  SET_VECTOR_ELT(contents, 57, ScalarReal(internal->toff));
  SET_VECTOR_ELT(contents, 58, ScalarReal(internal->ton));
  SET_VECTOR_ELT(contents, 59, ScalarReal(internal->treatment_failure_rate));
  SET_VECTOR_ELT(contents, 60, ScalarLogical(internal->model_use_dde));
  SEXP nms = PROTECT(allocVector(STRSXP, 61));
  ...
  SET_STRING_ELT(nms, 43, mkChar("resistance_asymptomatic_ratio"));
  SET_STRING_ELT(nms, 44, mkChar("resistance_baseline_ratio"));
  SET_STRING_ELT(nms, 45, mkChar("resistance_cleared_ratio"));
  SET_STRING_ELT(nms, 46, mkChar("resistance_dur_mult"));
  SET_STRING_ELT(nms, 47, mkChar("resistance_failed_ratio"));
  SET_STRING_ELT(nms, 48, mkChar("resistance_trans_mult"));
  SET_STRING_ELT(nms, 49, mkChar("rT_r_cleared"));
  SET_STRING_ELT(nms, 50, mkChar("rT_r_failed"));
  SET_STRING_ELT(nms, 51, mkChar("rT_s"));
  SET_STRING_ELT(nms, 52, mkChar("S0"));
  SET_STRING_ELT(nms, 53, mkChar("Sv0"));
  SET_STRING_ELT(nms, 54, mkChar("T_r_cleared0"));
  SET_STRING_ELT(nms, 55, mkChar("T_r_failed0"));
  SET_STRING_ELT(nms, 56, mkChar("T_s0"));
  SET_STRING_ELT(nms, 57, mkChar("toff"));
  SET_STRING_ELT(nms, 58, mkChar("ton"));
  SET_STRING_ELT(nms, 59, mkChar("treatment_failure_rate"));
  SET_STRING_ELT(nms, 60, mkChar("model_use_dde"));
...
SEXP model_set_user(SEXP internal_p, SEXP user) {
  ...
  internal->resistance_asymptomatic_ratio = user_get_scalar_double(user, "resistance_asymptomatic_ratio", internal->resistance_asymptomatic_ratio, NA_REAL, NA_REAL);
...
void model_rhs(model_internal* internal, double t, double * state, double * dstatedt, double * output) {
  ...
  double cA_r = internal->cA * (t > internal->ton && t < internal->toff ? internal->resistance_asymptomatic_ratio : 1);
  ...
  double rT_r_cleared_on = (t > internal->ton && t < internal->toff ? internal->rT_r_cleared : internal->rT_s);
  double rT_r_failed_on = (t > internal->ton && t < internal->toff ? internal->rT_r_failed : internal->rT_s);
  double treatment_failure_rate_on = (t > internal->ton && t < internal->toff ? internal->treatment_failure_rate : 0);
  dstatedt[5] = invading_A_r + (S * lambda_r * (1 - internal->phi) + lambda_r * A_s * (1 - internal->phi) + D_r * rD_r + T_r_failed * rT_r_failed_on - lambda_r * A_r * internal->phi * (1 - internal->ft) - lambda_r * A_r * internal->phi * internal->ft - lambda_s * A_r * internal->phi * (1 - internal->ft) - lambda_s * A_r * internal->phi * internal->ft - lambda_s * A_r * (1 - internal->phi) - A_r * rA_r);
  ...
  dstatedt[0] = (-(S) * lambda_s * (internal->phi * internal->ft + internal->phi * (1 - internal->ft) + (1 - internal->phi)) - S * lambda_r * (internal->phi * internal->ft + internal->phi * (1 - internal->ft) + (1 - internal->phi)) + T_s * internal->rT_s + A_s * internal->rA + A_r * internal->rA + T_r_cleared * rT_r_cleared_on);
  double invading_T_r_cleared = (t < internal->res_time || t > (internal->res_time + 1) ? 0 : T_s * log(1 / (double) (1 - internal->init_res)) * (1 - treatment_failure_rate_on));
  double invading_T_r_failed = (t < internal->res_time || t > (internal->res_time + 1) ? 0 : T_s * log(1 / (double) (1 - internal->init_res)) * treatment_failure_rate_on);
  double lambda_v_r = internal->a * (cA_r * A_r + cD_r * D_r + cT_r_cleared * T_r_cleared + cT_r_failed * T_r_failed);
  double lambda_v_s = internal->a * (internal->cA_s * A_s + internal->cD_s * D_s + internal->cT_s * T_s);
  ...
  dstatedt[6] = invading_T_r_cleared + (S * lambda_r * internal->phi * internal->ft * (1 - treatment_failure_rate_on) + lambda_r * A_r * internal->phi * internal->ft * (1 - treatment_failure_rate_on) + lambda_r * A_s * internal->phi * internal->ft * (1 - treatment_failure_rate_on) - T_r_cleared * rT_r_cleared_on);
  dstatedt[7] = invading_T_r_failed + (S * lambda_r * internal->phi * internal->ft * treatment_failure_rate_on + lambda_r * A_r * internal->phi * internal->ft * treatment_failure_rate_on + lambda_r * A_s * internal->phi * internal->ft * treatment_failure_rate_on - T_r_failed * rT_r_failed_on);
